...
def sample_random_subgrids(RR_pixels, ZZ_pixels, nr=64, nz=64):
    delta_r_min = 0.33 * (RR_pixels.max() - RR_pixels.min())
    delta_r_max = 0.75 * (RR_pixels.max() - RR_pixels.min())

    delta_z_min = 0.2 * (ZZ_pixels.max() - ZZ_pixels.min())
    delta_z_max = 0.75 * (ZZ_pixels.max() - ZZ_pixels.min())
...
def interp_fun(f, RR_pixels, ZZ_pixels, rr_grid, zz_grid, kind="quintic"):
    ...
    f_int = interp_func(rr_grid[0, :].ravel(), zz_grid[:, 0].ravel()).reshape(
        rr_grid.shape
    )
    return f_int
