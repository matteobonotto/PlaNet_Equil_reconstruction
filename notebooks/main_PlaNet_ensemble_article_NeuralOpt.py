...
from sklearn.metrics import r2_score, f1_score, accuracy_score, roc_curve, roc_auc_score, mean_squared_error
...
r_boundary_XP = np.transpose(np.squeeze(DB_separatrix_100_test_ConvNet[XP_YN==1,:,0]))
z_boundary_XP = np.transpose(np.squeeze(DB_separatrix_100_test_ConvNet[XP_YN==1,:,1]))
r_boundary_lim = np.transpose(np.squeeze(DB_separatrix_100_test_ConvNet[XP_YN==0,:,0]))
z_boundary_lim = np.transpose(np.squeeze(DB_separatrix_100_test_ConvNet[XP_YN==0,:,1]))
...
for i in range(12):
    coils_geo_r[i, :] = np.array([r[i]-.5*dr[i],
                                 r[i]+.5*dr[i],
                                 r[i]+.5*dr[i],
                                 r[i]-.5*dr[i]]).ravel()

    coils_geo_z[i, :] = np.array([z[i]-.5*dz[i],
                                 z[i]-.5*dz[i],
                                 z[i]+.5*dz[i],
                                 z[i]+.5*dz[i]]).ravel()
...
plt.fill(r_boundary_XP[:,ind_plot],z_boundary_XP[:,ind_plot],'C01', label = '$\Omega_p$')
plt.plot(r_boundary_XP[:,ind_plot],z_boundary_XP[:,ind_plot],'k', label = '$\Gamma_p$')
plt.fill(np.transpose(coils_geo_r[0,:]),np.transpose(coils_geo_z[0,:]),'gray', label = 'coils')
for i in range(2,12):
...
def fun_contourf_sol(z,RR,ZZ,sep = np.empty((1,2,)),titlefig = '',axis=None):
    plt.figure()
...
def fun_contour_sol(z,RR,ZZ,sep = np.empty((1,2,)),titlefig = ''):
    plt.figure()
...
def fun_contour_compare_sol(z_ref,z,RR,ZZ,sep = np.empty((1,2,)),titlefig = '',ncont = 10):
...
def fun_inpolygon(points, curve):
    ...
    if len(points_R.shape) == 2:
        mask = np.reshape(p.contains_points(np.column_stack((points_R.ravel(),
                                                             points_Z.ravel()))),
                          (points_R.shape[0], points_Z.shape[1]))

    else:
        mask = p.contains_points(np.column_stack((points_R.ravel(),
                                                            points_Z.ravel())))
    return mask
...
folder = os.path.join(r"/Users/matte/Dropbox/PhD/RESEARCH_ACTIVITY/ML/Equilibrium_ML/trained_models")

mat_history_j = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_Equil_kin_65588sample')) 
mat_history_mag = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_Equil_65588sample_5283epochs.h5')) 

folder = os.path.join(r"/Users/matte/Dropbox/PhD/RESEARCH_ACTIVITY/ML/Equilibrium_ML/Physics_Informed_AI_plasma_equilibrium/Notebooks")
mat_history_p = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_Equil_kin_65588_2')) 
...
plt.plot(np.arange(0,len(mat_history_p)),mat_history_p, label = 'mag + pressure + current')
plt.plot(np.arange(0,len(mat_history_j)),mat_history_j, label = 'mag + pressure + current')
plt.legend()
plt.xlabel('epochs')
plt.ylabel('loss')
...
mat_history_kin = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_Equil_kin_65588sample'))['history']
mat_history_mag = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_Equil_65588sample_5283epochs.h5'))['history']

mat_history_kin_p = scipy.io.loadmat(r'./Physics_Informed_AI_plasma_equilibrium/notebooks/history_PlaNet_Equil_kin_65588sample')['history']
mat_history_kin_fp = scipy.io.loadmat(r'./Physics_Informed_AI_plasma_equilibrium/notebooks/history_PlaNet_Equil_kin_p_65588sample_1000epochs.h5')['history']

mat_history_kin_p_2 = scipy.io.loadmat(r'./Physics_Informed_AI_plasma_equilibrium/notebooks/./history_PlaNet_Equil_kin_65588_2')['history']
...
def plot_loss(loss,epoch_0=0,agg_factor=10,label=''):
    df = pd.DataFrame(data = {
        'epochs_':np.arange(loss.shape[1])+epoch_0,
        'loss':loss.ravel()})
...
model_filename = os.path.join(r'./Physics_Informed_AI_plasma_equilibrium/model_PlaNet_NeuralOp_65588_3500epochs.h5')
...
r2_score_psi_kin_train = r2_score(vec_psi_ref_train, vec_psi_kin_train) #0.9999826736615185
r2_score_GSope_kin_train = r2_score(vec_GSope_ref_train, vec_GSope_kin_train) #0.9997634814110631

mse_psi_kin_train = mean_squared_error(vec_psi_ref_train, vec_psi_kin_train) # 0.00085132074
mse_GSope_kin_train = mean_squared_error(vec_GSope_ref_train, vec_GSope_kin_train) # 0.0037826695
...
r2_score_psi_kin_test = r2_score(vec_psi_ref_test, vec_psi_kin_test) #0.9999825504177072
r2_score_GSope_kin_test = r2_score(vec_GSope_ref_test, vec_GSope_kin_test) #0.9997612930908663

mse_psi_kin_test = mean_squared_error(vec_psi_ref_test, vec_psi_kin_test) # 0.00085321494
mse_GSope_kin_test = mean_squared_error(vec_GSope_ref_test, vec_GSope_kin_test) # 0.0037938117

s_scatter = 5
n_sup = int(.03*vec_GSope_ref_test.shape[0])
# plt.figure()
...
plt.figure()
plt.scatter(vec_GSope_ref_test[:n_sup],vec_GSope_kin_test[:n_sup], s_scatter, color = 'C2',
            marker = '.',label = 'val. - $R^2$ = {:4.4f}'.format(r2_score_GSope_kin_test))
plt.scatter(vec_GSope_ref_train[:n_sup],vec_GSope_kin_train[:n_sup], s_scatter, color =  'C3',
            marker = '.',label = 'train - $R^2$ = {:4.4f}'.format(r2_score_GSope_kin_train))
plt.gca().set_aspect('equal', adjustable='box')
plt.legend()
plt.gca().set_xlabel('$\Delta^*\psi$')
plt.gca().set_ylabel('$\Delta^*\psi^p$')
...
if printfig == True:
    for i in range(5):
        ...
        fun_contour_compare_sol(psi_NN_kin_ii,
                                psi_ref_ii,
                                RR_pixels,
                                ZZ_pixels,
                                separatrix_ref_ii,
                                ...
                                20)
        if printfig:
            plt.savefig('Psi_kin_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
       
        fun_contour_compare_sol(psi_NN_mag_ii,
                                psi_ref_ii,
                                RR_pixels,
                                ZZ_pixels,
                                separatrix_ref_ii,
                                ...
                                20)
        if printfig:
            plt.savefig('Psi_mag_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        ...
        printfig = True
        plt = fun_contour_compare_sol(GS_ope_ref_ii,
                                GSope_NN_kin_ii,
                                RR_in,
                                ZZ_in,
                                separatrix_ref_ii,
                                'GS op. (mag + profiles)')
        if printfig:
            plt.savefig('GS_ope_kin_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        ...
        plt = fun_contour_compare_sol(GS_ope_ref_ii,
                                ...
                                'GS op. (mag only)')
        if printfig:
            plt.savefig('GS_ope_mag_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        
        
        plt = fun_contourf_sol(100*np.abs(psi_NN_kin_ii - psi_ref_ii)/np.max(np.abs(psi_ref_ii)),
                         RR_pixels,
                         ZZ_pixels,
                         separatrix_ref_ii,
                         'Poloidal flux error [%] (mag + profiles)')
        if printfig:
            plt.savefig('Psi_kin_error_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        
        
        plt = fun_contourf_sol(100*np.abs(psi_NN_mag_ii - psi_ref_ii)/np.max(np.abs(psi_ref_ii)),
                         RR_pixels,
                         ZZ_pixels,
                         separatrix_ref_ii,
                         'Poloidal flux error [%] (mag only)')
        if printfig:
            plt.savefig('Psi_mag_error_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        
        
        plt = fun_contourf_sol(100*np.abs(GSope_NN_kin_ii - GS_ope_ref_ii)/np.max(np.abs(GS_ope_ref_ii)),
                         RR_in,
                         ZZ_in,
                         separatrix_ref_ii,
                         'GS op. [%] (mag + profiles)')
        if printfig:
            plt.savefig('GS_ope_kin_error_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
        
        
        plt = fun_contourf_sol(100*np.abs(GSope_NN_mag_ii - GS_ope_ref_ii)/np.max(np.abs(GS_ope_ref_ii)),
                         RR_in,
                         ZZ_in,
                         separatrix_ref_ii,
                         'GS op. [%] (mag only)')
        if printfig:
            plt.savefig('GS_ope_mag _error_case{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300)  
...
if printfig == True:
    for i in range(0,5):
        ...
        fun_contour_compare_sol(psi_ref_ii,
                                psi_NN_kin_ii,
                                RR_pixels,
                                ZZ_pixels,
                                separatrix_ref_ii,
        ...
        plt.savefig('example_Eq_rec_kin{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300) 
        ...
        fun_contour_compare_sol(psi_ref_ii,
                                ...
                                separatrix_ref_ii,
                                'Poloidal flux (mag only)')
        plt.savefig('example_Eq_rec_mag{:d}.png'.format(ind_plot), bbox_inches='tight', dpi=300) 
...
PlaNet_model_XPlim = tf.keras.models.load_model(os.path.join(folder + r'/model_PlaNet_XPlim_65588sample.h5'))
history_XPlim = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_XPlim_65588sample.h5'))['history']
...
print('accuracy train =', accuracy_score(XP_YN_train, out_clas_train)) #0.989632249801793
print('f1 test =', f1_score(XP_YN_test, out_clas_test)) #0.9883639422402916
print('accuracy test =', accuracy_score(XP_YN_test, out_clas_test)) #0.9898768142456397
...
plt.plot(FPR_test, TPR_test, '--', label = "val., ROC area = {:5.5f}".format(roc_auc_test))
...
s_scatter = 5
plt.figure()
plt.scatter(Psi_b_train_scaled,out_regr_train, s_scatter, color='C02', 
            marker = '.', label = 'train - $R^2$ = {:4.4f}'.format(R2_regr_Psi_b_train))
plt.scatter(Psi_b_test_scaled,out_regr_test, s_scatter, color='C03', 
            marker = '.', label = 'val. - $R^2$ = {:4.4f}'.format(R2_regr_Psi_b_test))
plt.axis('equal')
...
for i in range(10):
    ...
    plt.show()
    if printfig == True:
        plt.savefig('example_misclassification_{:d}.png'.format(ind_sel), bbox_inches='tight', dpi=300)
...
input_PlaNet_Separatrix_XP_train = [Psi_train_XP_scaled,
                                    Psi_b_train_XP_scaled]
input_PlaNet_Separatrix_XP_test = [Psi_test_XP_scaled,
                                   Psi_b_test_XP_scaled]


input_PlaNet_Separatrix_lim_train = [Psi_train_lim_scaled,
                                    Psi_b_train_lim_scaled]
input_PlaNet_Separatrix_lim_test = [Psi_test_lim_scaled,
                                   Psi_b_test_lim_scaled]
...
PlaNet_separatrix_lim = tf.keras.models.load_model(os.path.join(folder + r'/model_PlaNet_boundary_limiter_65588sample.h5'))
PlaNet_separatrix_XP = tf.keras.models.load_model(os.path.join(folder + r'/model_PlaNet_boundary_Xpoint_65588sample.h5'))

history_separatrix_lim = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_boundary_limiter_65588sample.h5'))['history']
history_separatrix_XP = scipy.io.loadmat(os.path.join(folder + r'/history_PlaNet_boundary_Xpoint_65588sample.h5'))['history']

history_separatrix_lim = history_separatrix_lim.ravel()
history_separatrix_XP = history_separatrix_XP.ravel()

plt.figure()
plt.plot(np.arange(0,len(history_separatrix_lim)),history_separatrix_lim, label = 'Limiter')
...
def fun_compare_separatrix(Psi_grid,separatrix_ref,separatrix_NN):
    ...
    # plt.colorbar()
    ind = np.row_stack((np.arange(0, separatrix_ref.shape[0]).reshape(-1, 1),
...
print('R2 train on R coordinate -> ' ,r2_score(Separatrix_ref_XP_train[:,:,0],
                                               Separatrix_XP_train[:,:,0])) 
print('R2 test on R coordinate -> ' ,r2_score(Separatrix_ref_XP_test[:,:,0],
                                              Separatrix_XP_test[:,:,0])) 
print('R2 train on Z coordinate -> ' ,r2_score(Separatrix_ref_XP_train[:,:,1],
                                               Separatrix_XP_train[:,:,1])) 
print('R2 test on Z coordinate -> ' ,r2_score(Separatrix_ref_XP_test[:,:,1],
                                              Separatrix_XP_test[:,:,1])) 

print('MSE train on R coordinate -> ' ,mean_squared_error(Separatrix_ref_XP_train[:,:,0],
                                               Separatrix_XP_train[:,:,0])) 
print('MSE test on R coordinate -> ' ,mean_squared_error(Separatrix_ref_XP_test[:,:,0],
                                              Separatrix_XP_test[:,:,0])) 
print('MSE train on Z coordinate -> ' ,mean_squared_error(Separatrix_ref_XP_train[:,:,1],
                                               Separatrix_XP_train[:,:,1])) 
print('MSE test on Z coordinate -> ' ,mean_squared_error(Separatrix_ref_XP_test[:,:,1],
                                              Separatrix_XP_test[:,:,1])) 
...
print('R2 train on R coordinate -> ' ,r2_score(Separatrix_ref_lim_train[:,:,0],
                                               Separatrix_lim_train[:,:,0])) 
print('R2 test on R coordinate -> ' ,r2_score(Separatrix_ref_lim_test[:,:,0],
                                              Separatrix_lim_test[:,:,0])) 
print('R2 train on Z coordinate -> ' ,r2_score(Separatrix_ref_lim_train[:,:,1],
                                               Separatrix_lim_train[:,:,1])) 
print('R2 test on Z coordinate -> ' ,r2_score(Separatrix_ref_lim_test[:,:,1],
                                              Separatrix_lim_test[:,:,1])) 

print('R2 train on R coordinate -> ' ,mean_squared_error(Separatrix_ref_lim_train[:,:,0],
                                               Separatrix_lim_train[:,:,0])) 
print('R2 test on R coordinate -> ' ,mean_squared_error(Separatrix_ref_lim_test[:,:,0],
                                              Separatrix_lim_test[:,:,0])) 
print('R2 train on Z coordinate -> ' ,mean_squared_error(Separatrix_ref_lim_train[:,:,1],
                                               Separatrix_lim_train[:,:,1])) 
print('R2 test on Z coordinate -> ' ,mean_squared_error(Separatrix_ref_lim_test[:,:,1],
                                              Separatrix_lim_test[:,:,1])) 
