...
def load_data(config):
    train_ds = (
        tf.data.Dataset.load(config["data"]["path"]["train"])
        .batch(config["data"]["batch_size"], drop_remainder=True)
        .shuffle(buffer_size=70000, seed=42, reshuffle_each_iteration=True)
    )
    test_ds = tf.data.Dataset.load(config["data"]["path"]["test"]).batch(
        config["data"]["batch_size"], drop_remainder=True
    )
