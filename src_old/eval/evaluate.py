...
def compute_GSO_kernels(RR_ds, ZZ_ds):
    ...
    for i in tqdm(range(ZZ_ds.shape[0])):
        ...
        alfa = -2 * (hr**2 + hz**2)
        Laplace_kernel_i = np.array(
            (
                [0, hr**2 / alfa, 0],
                [hz**2 / alfa, 1, hz**2 / alfa],
                [0, hr**2 / alfa, 0],
            )
        )
        Df_dr_kernel_i = (
...
class Evaluator:
    ...
    def run(self):
        ...
        if "mat" in self.config["data"]["path"]["grid"]:
            ...
            RR_ds = tf.tile(tf.expand_dims(RR_ds, axis=0), (x_ds.shape[0], 1, 1))
            ZZ_ds = tf.tile(tf.expand_dims(ZZ_ds, axis=0), (x_ds.shape[0], 1, 1))
